...
with colTitel_1:
    st.image("https://raw.githubusercontent.com/Gero-24-7/images/main/Logo.png")
#with colTitel_2:
    #("")
...
@st.experimental_dialog("weitere Programme")
...
with tab1:
    ...
    with colSystem_1:
        ...
        if roofForce_field == 0:
            st.write (":red[Fehler: Die ständige Last (g) darf nicht null sein, sonst wird Wert für g = 1 gesetzt.]")
            roofForce_field = 1.0
        ...
        with st.expander("Benutzerdefinierte Lasten (p)"):
            ...
            st.write("Beachte: Flächenlasten werden mit dem Lasteinzugsfelds verrechnet")
        ...
        # Ergebnis anzeigen
        # st.markdown(f":black[Die Streckenlast beträgt somit **{combinedForceField} kN/m**. (provisorische Anzeige)]")
    ...
    with colSystem_2:
        ...
        def draw_mass_band(beam_fields, ax):
            ...
            offset_horizontal = total_length/-10 #für Maßband der Einzelfelder
            offset_vertical = total_length/-7 #für Maßband gesamt 

            # Zeichne Maßbänder entlang dem Träger
            for i in range(len(node_positions)):
                ax.plot(node_positions[i], offset_horizontal, "k+", markersize=10)
                if i < len(node_positions) - 1:
                    ax.plot([node_positions[i], node_positions[i + 1]], [offset_horizontal, offset_horizontal], "k-", linewidth=1)
                    mid_point = (node_positions[i] + node_positions[i + 1]) / 2
                    if total_length >= 6:
                        # Zeichne die Länge unter der Maßkette
                        ax.annotate(f"{float(beam_fields[i]):.2f}m", (mid_point, offset_horizontal * 1.23), xytext=(0, 0), textcoords='offset points', ha='center', va='center', fontsize=8, annotation_clip=False)
                        # Zeichne die Feldnummer über der Maßkette
                        ax.annotate(f"L{i+1}", (mid_point, offset_horizontal * 0.85), xytext=(0, 0), textcoords='offset points', ha='center', va='center', fontsize=8, annotation_clip=False)
                    else:
                        # Zeichne die Länge unter der Maßkette
                        ax.annotate(f"{float(beam_fields[i]):.2f}m", (mid_point, offset_horizontal * 1.3), xytext=(0, 0), textcoords='offset points', ha='center', va='center', fontsize=8, annotation_clip=False)
                        # Zeichne die Feldnummer über der Maßkette
                        ax.annotate(f"L{i+1}", (mid_point, offset_horizontal * 0.7), xytext=(0, 0), textcoords='offset points', ha='center', va='center', fontsize=8, annotation_clip=False)

            # Zeichne Maßband für Gesamtlänge
            ax.plot(node_positions[0], offset_vertical, "k+", markersize=10)
            ax.plot(node_positions[-1], offset_vertical, "k+", markersize=10)
            ax.plot([node_positions[0], node_positions[-1]], [offset_vertical, offset_vertical], "k-", linewidth=1)
            if total_length >= 6:
                ax.annotate(f"{total_length:.2f}m", ((node_positions[0] + node_positions[-1]) / 2, offset_vertical * 1.15), xytext=(0, 0), textcoords='offset points', ha='center', va='center', fontsize=8, annotation_clip=False)
            else:
                ax.annotate(f"{total_length:.2f}m", ((node_positions[0] + node_positions[-1]) / 2, offset_vertical * 1.25), xytext=(0, 0), textcoords='offset points', ha='center', va='center', fontsize=8, annotation_clip=False)
...
# In jenem Verfahren werden die Feldmomente in der Liste moments gespeichert. Im aktuellen Code gibt es diesen Abschnitt aber nicht mehr
...
with tab2:
    ...
    with st.container(border=True):
        ...
        with colKräfte_1:
            ...
            if are_fields_different:
                ...
                if len(span_lengths) == 2:
                    ...
                    if ratio_l2_l1 <= 1.15:
                        ...
                        st.error("Das Verhältnis l1/l2 ist zu klein für die Tafelwerte. Die Berechnung kann so nicht durchgeführt werden.")
                        st.stop()
                    ...
                    st.write(f"Das maximale Stützmoment M (B) beträgt: {min_Mb:.2f} kNm")
                    ...
                    @st.experimental_dialog("Rechenweg für Momente (über L1/L2-Verhältnis)")
                    def show_popup():
                ...
                if len(span_lengths) == 3:
                    ...
                    # Ausgabe der berechneten Momente
                    st.write(f"Das maximale Stützmoment M (B) beträgt: {Mb_q_case7:.2f} kNm")
                    st.write(f"Das maximale Stützmoment M (C) beträgt: {Mc_q_case8:.2f} kNm")
                    st.write(f"Das größte Feldmoment M 1 beträgt: {max_M1:.2f} kNm")
                    st.write(f"Das größte Feldmoment M 2 beträgt: {max_M2:.2f} kNm")
                    ...
                    @st.experimental_dialog("Rechenweg für Momente und Querkräfte (über Formeln)")
                    def show_popup():
                ...
                # Auswahl der passenden Spalte basierend auf dem Verhältnis
                if ratio_pg >= 3.0:
                    st.error("Verhältnis p/g zu groß für die Tabellenwerte")
                    st.stop()
                elif ratio_pg >= 1.75:
                    selected_column = 0
                elif ratio_pg >= 1.25:
                ...
                if len(span_lengths) == 2:
                    ...
                    # Ausgabe der berechneten Momente
                    st.write(f"Das maximale Stützmoment M (B) beträgt: -{moment_M_b:.2f} kNm")
                ...
                if len(span_lengths) == 3:
                    ...
                    # Ausgabe der berechneten Momente
                    st.write(f"Das maximale Stützmoment M (B) und M (C) beträgt: -{moment_M_b:.2f} kNm")
                ...
                if len(span_lengths) == 4:
                    ...
                    # Ausgabe der berechneten Momente
                    st.write(f"Das maximale Stützmoment M (B) und M (D) beträgt: -{moment_M_b:.2f} kNm")
                    st.write(f"Das maximale Stützmoment M (C) beträgt: -{moment_M_c:.2f} kNm")
                ...
                if len(span_lengths) == 5:
                    ...
                    # Ausgabe der berechneten Momente
                    st.write(f"Das maximale Stützmoment M (B) und M (E) beträgt: -{moment_M_b:.2f} kNm")
                    st.write(f"Das maximale Stützmoment M (C) und M (D) beträgt: -{moment_M_c:.2f} kNm")
                ...
                # Popup-Funktion zur Anzeige des Rechenwegs
                @st.experimental_dialog("Rechenweg für Momente (über p/g-Verhältnis)")
                def show_popup():
                    ...
                    if len(span_lengths) == 2:
                        ...
                        st.markdown(f"**n-Wert für das maximale Stützmoment M (B):** {n_value_M_b:.2f}")
                        st.markdown(f"**n-Wert für das größte Feldmoment M 1:** {n_value_M_a_b:.2f}")
                        st.markdown(f"**Maximales Stützmoment M (B):** -{moment_M_b:.2f} kNm (Berechnet als: q * l^2 / n)")
                        st.markdown(f"**Größtes Feldmoment M 1:** {moment_M_a_b:.2f} kNm (Berechnet als: q * l^2 / n)")

                    # Zeige den Rechenweg für den Dreifeldträger
                    if len(span_lengths) == 3:
                        st.markdown(":blue[3-Feld-Träger Berechnung]")
                        st.markdown(f"**n-Wert für das maximale Stützmoment M (B) und M (C):** {n_value_M_b:.2f}")
                        ...
                        st.markdown(f"**Maximales Stützmoment M (B) und M (C):** -{moment_M_b:.2f} kNm (Berechnet als: q * l^2 / n)")
                        st.markdown(f"**Größtes Feldmoment M 1 und M 3:** {moment_M_a_b:.2f} kNm (Berechnet als: q * l^2 / n)")
                        st.markdown(f"**Größtes Feldmoment M 2:** {moment_M_b_c:.2f} kNm (Berechnet als: q * l^2 / n)")

                    # Zeige den Rechenweg für den Vierfeldträger
                    if len(span_lengths) == 4:
                        st.markdown(":blue[4-Feld-Träger Berechnung]")
                        st.markdown(f"**n-Wert für das maximale Stützmoment M (B) und M (D):** {n_value_M_b:.2f}")
                        st.markdown(f"**n-Wert für das maximale Stützmoment M (C):** {n_value_M_c:.2f}")
                        ...
                        st.markdown(f"**Maximales Stützmoment M (B) und M (D):** -{moment_M_b:.2f} kNm (Berechnet als: q * l^2 / n)")
                        st.markdown(f"**Maximales Stützmoment M (C):** -{moment_M_c:.2f} kNm (Berechnet als: q * l^2 / n)")
                        st.markdown(f"**Größtes Feldmoment M 1 und M 4:** {moment_M_a_b:.2f} kNm (Berechnet als: q * l^2 / n)")
                        st.markdown(f"**Größtes Feldmoment M 3 und M 2:** {moment_M_c_d:.2f} kNm (Berechnet als: q * l^2 / n)")

                    # Zeige den Rechenweg für den Fünffeldträger
                    if len(span_lengths) == 5:
                        st.markdown(":blue[5-Feld-Träger Berechnung]")
                        st.markdown(f"**n-Wert für das maximale Stützmoment M (B) und M (E):** {n_value_M_b:.2f}")
                        st.markdown(f"**n-Wert für das maximale Stützmoment M (C) und M (D):** {n_value_M_c:.2f}")
                        ...
                        st.markdown(f"**Maximales Stützmoment M (B) und M (E):** -{moment_M_b:.2f} kNm (Berechnet als: q * l^2 / n)")
                        st.markdown(f"**Maximales Stützmoment M (C) und M (D):** -{moment_M_c:.2f} kNm (Berechnet als: q * l^2 / n)")
                        st.markdown(f"**Größtes Feldmoment M 1 und M 5:** {moment_M_a_b:.2f} kNm (Berechnet als: q * l^2 / n)")
                        st.markdown(f"**Größtes Feldmoment M 3:** {moment_M_c_d:.2f} kNm (Berechnet als: q * l^2 / n)")
                        st.markdown(f"**Größtes Feldmoment M 2 und M 4:** {moment_M_b_c:.2f} kNm (Berechnet als: q * l^2 / n)")
            ...
            # Skizze zeigen
            if len(span_lengths) == 2:
                st.write("Momentenverlauf :red[(allgemeine Skizze)]")
            ...
            with colQuerkraft_2:
                ...
                if are_fields_different:
                    if len(span_lengths) == 2:
                        ...
                        def show_popup():
                            st.markdown(":blue[Da unterschiedliche Feldbreiten vorliegen, berechne Querkräfte nach Bautabellen Seite 4.17]")
                            st.markdown(f"**Veränderliche Lasten (p):** {p:.2f} kN")
                            st.markdown(f"**Ständige Lasten (g):** {g:.2f} kN")
                            st.markdown(f"**Gesamte Last (q):** {q:.2f} kN")
                            st.markdown(f"**Feldbreite (l1):** {l1:.2f} m")
                    ...
                    if len(span_lengths) == 2 or len(span_lengths) == 3:  
                        ...
                        def draw_beam_Querkraft(beam_fields):
                            ...
                            if len(span_lengths) == 2:
                                ...
                                shear_values = [-max_A, -min_Vbl, -max_Vbr, max_C]

                                # Beschriftung der Querkraftverlaufspunkte
                                for i, (x, y) in enumerate(shear_positions[1:-1]):  # Ohne Start- und Endpunkt
                                    if y > total_length/70:  # Punkt oberhalb des Trägers
                                        ax.text(x, y + total_length/10, f'{shear_values[i]:.2f} kN', ha='center', color='black')
                            ...
                            if len(span_lengths) == 3:
                                ...
                                # Werte der Querkraftverlaufspunkte
                                shear_values = [-max_A, -min_Vbl, -max_Vbr, -min_Vcl, -max_Vcr, max_D]

                                # Beschriftung der Querkraftverlaufspunkte
                                for i, (x, y) in enumerate(shear_positions[1:-1]):  # Ohne Start- und Endpunkt
                                    if y > total_length/70:  # Punkt oberhalb des Trägers
                                        ax.text(x, y + total_length/30, f'{shear_values[i]:.2f} kN', ha='center', color='black')
                                    else:  # Punkt unterhalb des Trägers
                                        ax.text(x, y - total_length/30, f'{shear_values[i]:.2f} kN', ha='center', color='black')


                            shear_x, shear_y = zip(*shear_positions)

                            ax.plot(shear_x, shear_y, 'k-', linewidth=2, label='Querkraftverlauf')
                    ...
                    # st.image("https://raw.githubusercontent.com/Gero-24-7/images/main/Querkraftverlauf.png")
                    ...
                    # Popup-Funktion zur Anzeige des Rechenwegs
                    @st.experimental_dialog("Querkraftverlauf")
                    def show_popup():
                        ...
                        if len(span_lengths) == 2:
                            ...
                            st.markdown(f"**Auflagerkraft V(A) und V(C):** 0.375 * q * l = {V_a:.2f} kN")
                    ...
                    def draw_beam_Querkraft(beam_fields):
                        ...
                        if len(span_lengths) == 2:
                            ...
                            # Werte der Querkraftverlaufspunkte
                            shear_values = [-V_a, V_bl, -V_br, V_c]
                        ...
                        if len(span_lengths) == 3:
                            ...
                            # Werte der Querkraftverlaufspunkte
                            shear_values = [-V_a, V_bl, -V_br, V_cl, -V_cr, V_d]
                            ...
                            # Werte der Querkraftverlaufspunkte
                            shear_values = [-V_a, V_bl, -V_br, V_cl, -V_cr, V_dl, -V_dr, V_e]
                            ...
                            # Werte der Querkraftverlaufspunkte
                            shear_values = [-V_a, V_bl, -V_br, V_cl, -V_cr, V_dl, -V_dr, V_el, -V_er, V_f]
...
with tab3:
    ...
    with st.container(border=True):
        ...
        with colProfilauswahl_1:
            ...
            @st.experimental_dialog("verwendete Tabelle")
    ...
    with st.container(border=True):
        ...
        with colProfil_1:
            ...
            if are_fields_different and len(span_lengths) == 4:
                ...
                #st.image("https://raw.githubusercontent.com/Gero-24-7/images/main/Ausgewähltes-Profil.png")
                ...
                l = span_lengths[0]  # Nehme die erste Feldbreite (alle Feldbreiten sind gleich)
                M0 = q * l**2 / 8
                Mm = abs(M_max_moments2 / 2)
                st.write(f"Moment am Stützpunkt Mm = {Mm:.2f} kNm")
                if material in ["Baustahl S235 (St37)", "Baustahl S355 (St52)"]:
                    k0 = 15
                    km = 18
                ...
                # Popup-Funktion zur Anzeige des Rechenwegs
                @st.experimental_dialog("Rechenweg Nachweise")
                def show_popup():
                    ...
                    st.write(f"**gemitteltes Stützmoment (Mm):** {Mm:.2f} kNm")
                    ...
                    if found_profile:
                        ...
                        if sigma_ed <= sigma_Rd:
                            ...
                            if not are_fields_different:
                                ...
                                if I_erf <= I_vorh:
                                    st.write(":green[**Gebrauchstauglichkeitsnachweis erfüllt (sigma_ed ≤ sigma_Rd)**]")
                                else:
                                    st.write(":red[**Gebrauchstauglichkeitsnachweis nicht erfüllt**]")
                        else:
                            st.write(":red[**Spannungsnachweis nicht erfüllt**]")
            ...
            #st.image("https://raw.githubusercontent.com/Gero-24-7/images/main/Alternativprofil.png")
...
with tab4:
    ...
    with st.container(border=True):
        ...
        st.write(":red[Möglichkeit: Ändere eine Feldbreite minimal ab, um Berechnungsverfahren 3 für genauere Werte auszuführen]")
